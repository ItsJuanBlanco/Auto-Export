...
// Clave compartida solo para el watcher de NinjaTrader (NO es la Service Role Key).
// Generarla con algo como: openssl rand -hex 32
// Configurarla en Vercel -> Project Settings -> Environment Variables.
...
export default async function handler(req, res) {
  try {
    ...
    const { clientId, accounts = [], strategies = [], orders = [], executions = [] } = req.body || {};

    if (!clientId) {
      return send(res, 400, { error: 'clientId is required.' });
    }
